...
def build_mapping(chromosome, D, pop1, neu_weights_update, neu_weights_k, K, features):
    ...
    for v in range(0, D):
        ...
        index = [i[0] for i in sorted(enumerate(distance_test), key=lambda x:x[1])] ##Create a list of index contatining the index of distance_test in ascending order
        #print minimum_test
        for tt in range(0, len(distance_test)):
            if index[tt] in L:  ##If the minium distance is already present in L, thesn skip
                continue
            else:
                L.insert(v, index[tt])  ##Else, build mapping of the winning neuron by inserting the next closest neuron that is now present in L
...
def mapping(S, chromosome, population, tau, t, T,C,D,neu_weights, neu_weights_k, S_K, K, features):
    ...
    neu_weights_update, neu_weights_k = neuron_weights_update(S,chromosome, tau, D, C, t, T, neu_weights, neu_weights_k, S_K, features)  ##Updated neuron weights

    L, neu_weights_k = build_mapping(chromosome, D, pop1, neu_weights_update, neu_weights_k, K, features)  ##Mapping of the solution to the neurons
