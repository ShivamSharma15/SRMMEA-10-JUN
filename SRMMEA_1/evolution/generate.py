...
def mutate(a, b, y2, feat):
    ...
    for j in range(0, feat):
        ...
        if b[j] == a[j]:
            ...
            a[j] = 0
        if(r < 0.5):
            delta = math.pow((2*r) + ((1-(2*r)) * math.pow(((b[j]-y2[j])/(float(b[j]-a[j]))), (nm+1))),  (1/float(nm + 1))) - 1
            #print r, delta
        else:
            delta = 1 - math.pow((2-(2*r)) + (((2*r)-1) * math.pow(((y2[j]-a[j])/(float(b[j]-a[j]))), (nm+1))),  (1/float(nm + 1)))
            #print r, delta
